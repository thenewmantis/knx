#!/bin/sh
# knx: Read the Knox Bible from your terminal
...
while [ $# -gt 0 ]; do
	...
	if [ "$1" = "--" ]; then
		...
		get_data knx.tsv | awk -v cmd=list "$(get_data knx.awk)"
		exit
	elif [ "$1" = "-W" ]; then
...
if [ $# -eq 0 ]; then
	...
	while true; do
		...
		get_data knx.tsv | awk -v cmd=ref -v ref="$ref" "$(get_data knx.awk)" | ${PAGER}
	done
	exit 0
fi

get_data knx.tsv 2>/dev/null | awk -v cmd=ref -v ref="$*" "$(get_data knx.awk)" 2>/dev/null | ${PAGER}
